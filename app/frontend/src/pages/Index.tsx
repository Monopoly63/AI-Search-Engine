          ...
          <div style={{ opacity: fadeIn ? 1 : 0, transition: 'opacity 200ms ease' }}>
            <div className="bento-grid">
              <div className="bento-stack">
                ...
                {/* FRONT AND CENTER 12×12 GRID VISUALIZER (Zero Hidden Playgrounds) */}
                <section className="glass-card card-pad" style={{ marginBottom: 24, border: '1px solid var(--accent)', boxShadow: '0 0 35px var(--accent-soft)' }}>
                  <div className="section-heading" style={{ marginBottom: 18, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                    <div className="section-title-group">
                      <div className="section-kicker" style={{ color: 'var(--accent)', fontWeight: 800 }}>⭐ {isRTL ? 'بيئة التفاعل المباشرة (القلب النابض للمنصة)' : 'Front-and-Center Playground'}</div>
                      <h2 className="section-title" style={{ fontSize: 20 }}>{algoLabel[algo]} · 12 × 12 {isRTL ? 'شبكة الرسم والمحاكاة' : 'Grid Visualizer'}</h2>
                    </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 24, alignItems: 'center' }}>
                    <div className="grid-stage" style={{ margin: '0 auto', overflow: 'auto', maxWidth: '100%' }}>
                      <div className="grid-board" style={{ gridTemplateColumns: `repeat(${COLS}, ${cellSize})`, gridTemplateRows: `repeat(${ROWS}, ${cellSize})`, border: '2px solid var(--border-strong)', borderRadius: 14, overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, background: 'var(--bg-elevated)', padding: 20, borderRadius: 18, border: '1px solid var(--border)' }}>
                      <div>
                        <LabelRow label={t.speed} value={`${gridSpeed}ms`} />
                        <input className="np-range" type="range" min={10} max={300} step={5} value={gridSpeed} onChange={(e) => setGridSpeed(Number(e.target.value))} style={{ width: '100%', marginTop: 8 }} />
                      </div>
                      <Divider />
                ...
                <section className="glass-card card-pad" style={{ marginBottom: 24, border: '1px solid var(--accent)', boxShadow: '0 0 35px var(--accent-soft)' }}>
                  <div className="section-heading" style={{ marginBottom: 18, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                    <div className="section-title-group">
                      <div className="section-kicker" style={{ color: 'var(--accent)', fontWeight: 800 }}>⭐ {isRTL ? 'بيئة التفاعل المباشرة (القلب النابض للمنصة)' : 'Front-and-Center Interactive Canvas'}</div>
                      <h2 className="section-title" style={{ fontSize: 20 }}>{algoLabel[algo]} · 12 × 12 {isRTL ? 'شبكة الرسم والتتبع' : 'Grid Visualizer'}</h2>
                    </div>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      <GhostButton label={gridRun.running ? t.btnPause : t.btnPlay} solid={!gridRun.running} onClick={toggleGridPlayback} />
                      <GhostButton label={t.btnReset} onClick={resetGrid} />
